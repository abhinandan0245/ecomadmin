...
import { addProductThunk, getAllBrandsByCategoryThunk, getByIdProductThunk, updateProductThunk } from '../../store/thunks/productThunk';
...
const AddNew = () => {
    ...
    const [previews, setPreviews] = useState<{ src: string; isLocal: boolean }[]>([]);
    ...
    const handleSizeSelection = (selected: MultiValue<Option>) => {
        ...
        const newSizeVariants = selectedSizes
            ...
            .map((size) => ({
                ...
                price: calculateFinalPrice(form.originalPrice || '0', form.discountPercentage || '0', form.discountAmount || '0'),
            }));

        // Remove sizes that were deselected
        const updatedVariants = [...form.sizeVariants.filter((v) => selectedSizes.includes(v.size)), ...newSizeVariants];
    ...
    const updateSizeVariant = (index: number, field: keyof SizeVariant, value: string) => {
        ...
        if (field === 'originalPrice' || field === 'discountPercentage' || field === 'discountAmount') {
            updatedVariants[index].price = calculateFinalPrice(updatedVariants[index].originalPrice, updatedVariants[index].discountPercentage, updatedVariants[index].discountAmount);
        }
    ...
    useEffect(() => {
        if (id && selectedProduct) {
            ...
            if (selectedProduct.priceVariants) {
                const variants = Array.isArray(selectedProduct.priceVariants) ? selectedProduct.priceVariants : [selectedProduct.priceVariants];
            ...
            if (selectedProduct.sizes && selectedProduct.sizes.length > 0) {
                selectedSizes = Array.isArray(selectedProduct.sizes) ? selectedProduct.sizes : [selectedProduct.sizes];
            }
            ...
            setForm({
                ...
                brand: selectedProduct.brand || '',
                sizeVariants,
                productStatus: typeof selectedProduct.productStatus === 'boolean' ? (selectedProduct.productStatus ? 'active' : 'inactive') : selectedProduct.productStatus || 'active',
            });

            // Set tags
            if (selectedProduct.tags) {
                setTags(Array.isArray(selectedProduct.tags) ? selectedProduct.tags : typeof selectedProduct.tags === 'string' ? selectedProduct.tags.split(',') : []);
            } else {
                setTags([]);
            }
            ...
            if (selectedProduct.imageVariants) {
                const variants = Array.isArray(selectedProduct.imageVariants) ? selectedProduct.imageVariants : [selectedProduct.imageVariants];
    ...
    useEffect(() => {
        if (selectedProduct?.imageVariants) {
            const existingPreviews = selectedProduct.imageVariants.map((img:any) => ({
                src: img.imageUrl, // DB se full URL
                isLocal: false,
            }));
    ...
    const handleSubmit = async () => {
        ...
        // Add tags
        tags.forEach((tags) => formData.append('tags[]', JSON.stringify(tags)));
    ...
    const baseFinalPrice = calculateFinalPrice(form.originalPrice, form.discountPercentage, form.discountAmount);
                    ...
                    <div className="flex justify-between lg:flex-row flex-col">
                        <div className="lg:w-1/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                            ...
                            <div className="mt-4 flex items-center">
                                ...
                                <input value={form.title} onChange={handleInputChange} id="Title" type="text" name="title" className="form-input flex-1" placeholder="Enter Product Title" required />
                            </div>
                            <div className="mt-4 flex items-center">
                                <label htmlFor="HSNCode" className="ltr:mr-2 rtl:ml-2 w-1/3 mb-0">
                                    HSN Code
                                </label>
                                <input value={form.hsnCode} onChange={handleInputChange} id="HSNCode" type="text" name="hsnCode" className="form-input flex-1" placeholder="Enter HSN Code" />
                            </div>
                            ...
                            <div className="flex items-center mt-10">
                                ...
                                <input value={form.stock} onChange={handleInputChange} id="Stock" type="number" name="stock" className="form-input flex-1" placeholder="Enter Stock" min="0" />
                            </div>
                        </div>
                        <div className="lg:w-1/2 w-full">
                            <div className="flex items-center mt-4">
                                ...
                                <select value={form.categoryId} onChange={handleDropdownChange} id="category" name="categoryId" className="form-select flex-1" required>
                            ...
                            <div className="flex items-center mt-4">
                                ...
                                />
                                {sizesError && <div className="text-red-500 text-sm mt-1">{sizesError}</div>}
                            </div>
                                        ...
                                        <div key={index} className="mb-4 last:mb-0">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="form-input w-full bg-gray-100">₹{variant.price}</div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="flex items-center mt-4">
                                ...
                                <select value={form.brand} onChange={handleDropdownChange} id="brand" name="brand" className="form-select flex-1" required>
                                    <option value="">Select Brand</option>
                                    {(brands || [])
                                        .filter((brand) => {
                                            if (!form.categoryId) return true; // category select na ho to sab brands dikhao
                                            if (!brand.categoryId) return false;

                                            // brand.categoryId is string "36,34,33,37"
                                            const catIds = brand.categoryId.split(',').map((id: string) => id.trim());

                                            return catIds.includes(String(form.categoryId));
                                        })
                                        .map((brand) => (
                            ...
                            <div className="flex items-center mt-4">
                                ...
                                <select value={form.productStatus} onChange={handleDropdownChange} id="productStatus" name="productStatus" className="form-select flex-1" required>
...
{/* 
                    ...
                    <div className="flex flex-col lg:flex-col gap-6 mt-6">
                        <div className="mt-4 flex-1">
                            ...
                            <ReactQuill value={form.description} onChange={handleDescriptionChange} className="bg-white mb-4 h-64" theme="snow" />
                        </div>
                        <div className="mt-4 flex-1">
                            <label htmlFor="Ingredients" className="block mb-2">
                                Ingredients
                            </label>
                            <ReactQuill value={form.ingredients} onChange={handleIngredientsChange} className="bg-white mb-4 h-64" theme="snow" />
                        </div>
                        <div className="mt-4 flex-1">
                            <label htmlFor="AdditionalInfo" className="block mb-2">
                                Additional Info
                            </label>
                            <ReactQuill value={form.additionalInfo} onChange={handleAdditionalInfoChange} className="bg-white mb-4 h-64" theme="snow" />
                        </div>
                    </div>
                        ...
                        <div className="flex gap-3 flex-wrap">
                            {previews.map((preview, index) => (
                                <div key={index} className="relative">
                                    <img src={preview.src} alt={`preview-${index}`} className="w-24 h-24 object-cover rounded border" />
                                    <button onClick={() => removeImage(index)} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
                                        ✕
                                    </button>
                                </div>
                            ))}
            ...
            <div className="panel flex justify-center items-center">
                <button onClick={handleSubmit} className="btn btn-success w-52" disabled={!form.productId || !form.title || !form.categoryId || !form.brand}>
