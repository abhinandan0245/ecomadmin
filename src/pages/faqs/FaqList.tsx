...
const FaqList = () => {
    ...
    const handleAddOrUpdate = async () => {
        if (!title.trim() || !question.trim() || !answer.trim()) {
            return toast.error('Title, Question and Answer are required');
        }
        try {
            if (editingId !== null) {
                await updateFaq({ id: editingId, title, question, answer }).unwrap();
                toast.success('FAQ updated');
            } else {
                await createFaq({ title, question, answer }).unwrap();
        ...
        <div className="flex flex-col gap-4">
            <div className="panel">
                ...
                <div className="flex flex-col space-y-3">
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Section Title (e.g. Shopping Information)" className="form-input" />
                    <input type="text" value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Question" className="form-input" />
                    <textarea rows={5} value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder="Answer" className="form-textarea" />
                    <button type="button" className="btn btn-success w-max" onClick={handleAddOrUpdate}>
            ...
            <div className="panel">
                ...
                {isLoading ? (
                    ...
                    <DataTable
                        records={records}
                        columns={[
                           { accessor: 'title', sortable: true, title: 'Section Title' },
